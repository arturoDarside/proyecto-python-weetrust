...
import subprocess  # Para ejecutar el script
import ui_token
...
class MainApp(tk.Tk):
    ...
    def mostrar_token(self):
         """ Muestra la sección del token usando el panel personalizado """
    ...
    def mostrar_firmar(self):
      self.limpiar_frame()  # Limpia el contenido actual del frame principal
      tk.Label(self.content_frame, text="Sección: Firmar", font=("Arial", 14), bg='white').pack(pady=20)

    # Crear instancia de FirmarPanel y añadirla al content_frame
      firmar_panel = FirmarPanel(self.content_frame)
      firmar_panel.pack(fill="both", expand=True)  # Ajusta el tamaño del panel dentro del frame principal
    ...
    def mostrar_lector_correos(self):
     """ Muestra la sección del lector de correos """
     self.limpiar_frame()  # Limpia el contenido actual del frame principal
     lector_panel = LectorPanel(self.content_frame)
     lector_panel.pack(fill="both", expand=True)  # Ajusta el tamaño del panel dentro del frame principal
